...
args = EasyDict({
    ...
    'detector': "tracker",
    'headless': True,  # Set to True for running without GUI/display (e.g. testing)
...
def extractFrame(videoPath):
    ...
    if not args.singleTracker:
        ...
        if args.headless:
            # Default bounding boxes for LeBron (left/middle) and Jordan (right/middle) in 1280x720 video
            bboxes = [(350, 100, 150, 400), (600, 120, 150, 400)]
            colors = [(255, 0, 0), (0, 0, 255)]
            print("Running in headless mode. Using default bounding boxes: {}".format(bboxes))
        else:
...
def writeVideo(videoPath, videoFrames, playerBoxes, predictions, colors, frame_width=1280, frame_height=720, vid_stride=8):
    ...
    output_dir = os.path.dirname(videoPath)
    if output_dir:
